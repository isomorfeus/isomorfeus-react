...
RSpec.describe 'LucidPaper::Component' do
  it 'can render a component that is using inheritance' do
    doc = visit('/')
    doc.evaluate_ruby do
      class TestComponent < LucidPaper::Component::Base
        render do
  ...
  it 'can render a component that is using the mixin' do
    ...
    doc.evaluate_ruby do
      class TestComponent
        include LucidPaper::Component::Mixin
        render do
          DIV(id: :test_component) { 'TestComponent rendered' }
        end
      end
  ...
  context 'it can use callbacks like' do
    ...
    it 'component_did_catch' do
      @doc.evaluate_ruby do
        class ComponentWithError < LucidPaper::Component::Base
          def text
            'Error caught!'
          end
          render do
